...
cat $srcdir/wav.scp | awk -v p=$spk_prefix -v s=$spk_suffix '{printf("%s %s%s%s\n", $1, p, $1, s);}' > $destdir/wav_map
...
if [ -f $srcdir/segments ]; then
  utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/segments | \
   utils/apply_map.pl -f 2 $destdir/wav_map >$destdir/segments
  utils/apply_map.pl -f 1 $destdir/wav_map < $srcdir/wav.scp > $destdir/wav.scp
...
if [ -f $srcdir/reco2file_and_channel ]; then
  ...
  utils/apply_map.pl -f 1 $destdir/wav_map < $srcdir/reco2file_and_channel | \
  utils/apply_map.pl -f 2 $destdir/wav_map >$destdir/reco2file_and_channel
fi
...
if [ -f $srcdir/reco2dur ]; then
  if [ -f $srcdir/segments ]; then
    utils/apply_map.pl -f 1 $destdir/wav_map < $srcdir/reco2dur > $destdir/reco2dur
  else
...
rm $destdir/spk_map $destdir/utt_map $destdir/wav_map
